        ...
        public int SaveMaxCards(int addCards, string siteKey, string cardValue)
        {
            ...
            try
            {
                if (addCards != 0)
                {
                    if (addCards < 0)
                    {
                        int siteId = siteList.GetMasterSiteFromHQ().SiteId;
                        string numberFormat = ParafaitDefaultContainer.GetParafaitDefault(executionContext, "NUMBER_FORMAT");
                        ...
                        site = new Site(executionContext.GetSiteId(), null);
                        MaxCardsEncrypted = site.getSitedTO.MaxCards;
                        if (MaxCardsEncrypted != null)
                        {
                    ...
                    siteDTO = site.getSitedTO;
                    siteDTO.MaxCards = encrypted;
                    site = new Site(executionContext, siteDTO);
                    site.Save();
                    MaxCards += addCards;
                }
