...
{
    public class RedemptionStatusController : ApiController
    {
        ...
        /// <returns></returns>
        [HttpPost]
        [Route("api/Redemption/RedemptionOrder/{orderId}/Status")]
        [Authorize]
        public async Task<HttpResponseMessage> Post([FromUri] int orderId, [FromBody] RedemptionActivityDTO redemptionActivityDTO)
        {
            ...
            try
            {
                ...
                if (redemptionActivityDTO != null)
                {
                    ...
                    RedemptionDTO content = await redemptionUseCases.UpdateRedemptionStatus(orderId ,redemptionActivityDTO);
                    log.LogMethodExit();
