    ...
    public class MessagesController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> Get(string activityType = null, int currentPage = 0, int pageSize = 5, string messages = null)
        {
            try
            {
                ...
                return Request.CreateResponse(HttpStatusCode.OK, new { data = messagesDTOList, isProtectedReadonly = isProtected, currentPageNo = currentPage, TotalCount = totalNoOfMessages, MaxMessageNo = maxMessageNo, token = securityTokenDTO.Token });
            }
            catch (Exception ex)
            {
