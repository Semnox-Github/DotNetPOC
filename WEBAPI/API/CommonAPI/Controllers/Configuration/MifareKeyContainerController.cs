...
{
    public class MifareKeyContainerController : ApiController
    {
        ...
        [HttpGet]
        [Authorize]
        [Route("api/Configuration/MifareKeyContainer")]
        public async Task<HttpResponseMessage> Get(int siteId = -1, string hash = null, bool rebuildCache = false)
        {
            ...
            try
            {
                ...
                MifareKeyContainerDTOCollection defaultViewDTOCollection = await
                           Task<MifareKeyContainerDTOCollection>.Factory.StartNew(() => {
                                    if(rebuildCache)
                                    {
                                        MifareKeyViewContainerList.Rebuild(siteId);
                                    }
                                    return MifareKeyViewContainerList.GetMifareKeyContainerDTOCollection(siteId, hash);
                                  });
