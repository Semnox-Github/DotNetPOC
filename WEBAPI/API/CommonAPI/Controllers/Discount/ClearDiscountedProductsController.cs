 ...
 * Description  - API for the Clear Discounted Products Operation.
 *  
 **************
 **Version Log
 **************
...
{
    public class ClearDiscountedProductsController : ApiController
    {
        ...
        [HttpPost]
        [Route("api/Discount/ClearDiscountedProducts")]
        [Authorize]
        public async Task<HttpResponseMessage> Post([FromBody]int discountId)
        {
            ...
            try
            {
                ...
                await discountUseCases.ClearDiscountedProducts(discountId);
                log.LogMethodExit();
