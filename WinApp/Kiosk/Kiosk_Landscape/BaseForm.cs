...
namespace Parafait_Kiosk
{
    public partial class BaseForm : Form
    {
        ...
        public virtual void KioskTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (this == ActiveForm)
            {
                if (kioskSecondsRemaining <= 10)
                {
                    ...
                    {
                        PerformAbortAction(kioskTransaction);
                        //this.Close();
                        CloseForms();
                    }
                }
        ...
        public static void DirectCashAbortAction(KioskTransaction kioskTransaction)
        {
            ...
            if (KioskStatic.Utilities != null && KioskStatic.Utilities.ExecutionContext != null && kioskTransaction != null)
            {
                    ...
                    ) && (kioskTransaction.HasActiveTransactionLines() || trxAmount > 0 ))
                {
                    ...
                    string messageTwo = MessageContainerList.GetMessage(KioskStatic.Utilities.ExecutionContext, "Do you want to abort current transaction?");
                ...
                {
                    PerformAbortAction(kioskTransaction);//clear the transaction
                }
            }
            log.LogMethodExit();
        }
        private static void PerformAbortAction(KioskTransaction kioskTransaction)
        {
            log.LogMethodEntry("kioskTransaction");
            try
            {
        ...
        //public static bool formIsClosed = false;
        protected void CloseForms()
        {
            log.LogMethodEntry();
            //bool fskSalesEnabled = ParafaitDefaultContainerList.GetParafaitDefault<bool>(KioskStatic.Utilities.ExecutionContext, "KIOSK_ENABLE_SALES_OPTION", true);
            //bool fskExecuteOnlineTrxEnabled = ParafaitDefaultContainerList.GetParafaitDefault<bool>(KioskStatic.Utilities.ExecutionContext, "KIOSK_ENABLE_EXECUTE_TRANSACTION_OPTION", false);
            //bool singleFunctionMode = !(fskSalesEnabled && fskExecuteOnlineTrxEnabled);
            int lowerLimit = 0;// singleFunctionMode ? 1 : 0;
