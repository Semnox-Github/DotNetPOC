...
namespace Parafait_Kiosk
{
    public partial class FSKCoverPage : BaseForm
    {
        ...
        void InitKiosk()
        {
            ...
            SetKioskVariableTopUpProduct();
            tagNumberParser = new TagNumberParser(utilities.ExecutionContext);
            KioskStatic.get_config(KioskStatic.KIOSKMODEISCEC);
            SetTheme();
            SetClientLogo();
        ...
        private void SetTheme()
        {
            ...
            KioskStatic.formatMessageLine(txtMessage, 26, Properties.Resources.bottom_bar);// ThemeManager.CurrentThemeImages.BottomMessageLineImage);
            KioskStatic.setDefaultFont(this);
            this.BackgroundImage = KioskStatic.CurrentTheme.HomeScreenBackgroundImage;// ThemeManager.CurrentThemeImages.HomeScreenBackgroundImage;
            pbClientLogo.Image = KioskStatic.CurrentTheme.HomeScreenClientLogo; //ThemeManager.CurrentThemeImages.HomeScreenClientLogo;
            btnFSKSales.BackgroundImage = KioskStatic.CurrentTheme.FSKSalesButton;//ThemeManager.CurrentThemeImages.FSKSalesButton;
            btnExecuteOnlineTransaction.BackgroundImage = KioskStatic.CurrentTheme.ExecuteOnlineTrxButton;//ThemeManager.CurrentThemeImages.ExecuteOnlineTrxButton;
            ...
            if (KioskStatic.config.baport > 0)
            {
                try
                {
                    if (!KioskStatic.DisablePurchase)
                    {
                        ...
                        {
                            commonBillAcceptor = new CommonBillAcceptor(billAcceptorMonitor, KioskStatic.config.baport.ToString());
        ...
        private void FrmFSKCoverPage_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (singleFunctionMode)
                {
                    ...
                    {
                        MessageBox.Show(MessageContainerList.GetMessage(utilities.ExecutionContext, 1700));
                        Environment.Exit(1);
                    }
                }
        ...
        private void FrmFSKCoverPage_Deactivate(object sender, EventArgs e)
        {
            log.LogMethodEntry(sender, e);
            StopKioskTimer();
            log.LogMethodExit();
        ...
        private void FrmFSKCoverPage_Activated(object sender, EventArgs e)
        {
            log.LogMethodEntry(sender, e);
            try
            {
            ...
            {
                log.Error(ex);
                KioskStatic.logToFile("frmFSKCoverPage_Activated: " + ex.Message);
            }
            log.LogMethodExit();
        }
        ...
        private void HandleBillAcceptorEvent(KioskStatic.acceptance ac)
        {
