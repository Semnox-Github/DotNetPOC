 ...
 *********************************************************************************************
 *2.4.0       28-Sep-2018      Guru S A           Modified for device container, On line Transaction in Kiosk changes
 *2.70.2      10-Sep-2019      Deeksha            Waiver Phace 2 Enhancement changes
 *2.70.2      10-Nov-2019      Girish             Ticket printer integration
 ...
 *2.140.0     24-Nov-2021      Sathyavathi        CEC enhancement - Application Version Changes Reconciliation
 *2.150.0.0   23-Sep-2022      Sathyavathi        Check-In feature Phase-2
 *2.150.1     22-Feb-2023      Guru S A           Kiosk Cart Enhancements
 *2.155.0     10-Jun-2023      Sathyavathi        Attraction Sale in Kiosk
 ********************************************************************************************/
...
namespace Parafait_Kiosk
{
    public partial class frmHome : BaseForm
    {
        ...
        void InitKiosk()
        {
            ...
            tagNumberParser = new TagNumberParser(utilities.ExecutionContext);
            KioskHelper.CopyDefaultKioskStyles();
        ...
        private void frmHome_FormClosed(object sender, FormClosedEventArgs e)
        {
            ...
            string message = MessageContainerList.GetMessage(utilities.ExecutionContext, 5148); //"Exiting FSK Kiosk"
        ...
        private void frmHome_Load(object sender, EventArgs e)
        {
            ...
            string message = MessageContainerList.GetMessage(utilities.ExecutionContext, 5147); //"Launching FSK Kiosk"
        ...
        private void ArrangeDisplayButtonImg()
        {
            ...
            string fontFamName = (KioskStatic.CurrentTheme.DefaultFont != null ? KioskStatic.CurrentTheme.DefaultFont.FontFamily.Name : "Gotham Rounded Bold");
            Font largeButtonFont = new System.Drawing.Font(fontFamName, KioskStatic.CurrentTheme.HomeScreenOptionsLargeFontSize,
                                           System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(0)));
            Font smallButtonFont = new System.Drawing.Font(fontFamName, KioskStatic.CurrentTheme.HomeScreenOptionsSmallFontSize,
                                           System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(0)));
            Size smallBtnSize = new Size(421, 231);
            int btnCount = flpOptions.Controls.Count;

            if (btnCount == 13)
            {
                int heightAdjuster = (btnCount == 13 ? -80 : 0);
                //first 1 is big
                for (int i = 0; i < flpOptions.Controls.Count; i++)
                {
                    ...
                    if (type.Contains("button"))
                    {
                        ...
                        else
                        {
                            ...
                            SetButtonMargineForMore(i, c);
                        }
                        c.Size = new Size(c.BackgroundImage.Width, c.BackgroundImage.Height + heightAdjuster);
            ...
            if (btnCount == 12)
            {
                ...
                for (int i = 0; i < flpOptions.Controls.Count; i++)
                {
                    ...
                    if (type.Contains("button"))
                    {
                        ...
                        c.BackgroundImage = GetSmallBackgroundImage(c);
                        c.TextAlign = KioskStatic.GetContextAligment(KioskStatic.CurrentTheme.HomeSmallButtonTextAlignment);
                        SetSmallButtonMargine(i, c);
                        c.Font = smallButtonFont;
                        c.Size = smallBtnSize;// c.BackgroundImage.Size;
                    }
                }
            }
            else if (btnCount == 11 || btnCount == 9)
            {
                int heightAdjuster = (btnCount == 11 ? -40 : 0);
                //first 1 is big
                for (int i = 0; i < flpOptions.Controls.Count; i++)
                {
                    string type = flpOptions.Controls[i].GetType().ToString().ToLower();
            ...
            else if (btnCount == 10)
            {
                ...
                for (int i = 0; i < flpOptions.Controls.Count; i++)
                {
                    ...
                    if (type.Contains("button"))
                    {
                        ...
                        c.BackgroundImage = GetSmallBackgroundImage(c);
                        c.TextAlign = KioskStatic.GetContextAligment(KioskStatic.CurrentTheme.HomeSmallButtonTextAlignment);
                        SetButtonMargine(i, c);
                        c.Font = smallButtonFont;
                        c.Size = c.BackgroundImage.Size;
                    }
                }
            }
            else if (btnCount == 8 || btnCount == 6)
            {
                //first 2 are big
                for (int i = 0; i < flpOptions.Controls.Count; i++)
                {
        ...
        private void SetSmallButtonMargine(int buttonIndex, Button btnObject)
        {
            log.LogMethodEntry(buttonIndex, btnObject.Name);

            if (buttonIndex % 2 == 0) //Right side of the screen
            {
                btnObject.Margin = new Padding(75, 2, 0, 0);
            }
            else //left side of the screen
            {
                btnObject.Margin = new Padding(40, 2, 0, 0);
            }
            log.LogMethodExit();
        }
        ...
        private void DoDynamicDisplay()
        {
            ...
            {
                flpOptions.Size = new Size(KioskStatic.CurrentTheme.HomeScreenOptionWidth, flpOptions.Height - 100);
                int locX = (1080 / 2) - (flpOptions.Width / 2);
                flpOptions.Location = new Point(locX, flpOptions.Location.Y);
            }
            else
            {
                flpOptions.Size = new Size(flpOptions.Width, flpOptions.Height - 100);
            }
                ...
                foreach (HomeScreenOption item in homeScreenOptionDisplay.GetOptionList)
                {
                    if (item.CanShowTheOption())
                    {
                        try
                        {
                            ...
                            optionButton.Margin = new Padding(20, 20, 20, 0);
                            //optionButton.Padding = new Padding(2);
