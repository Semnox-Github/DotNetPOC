...
namespace Parafait_Kiosk
{
    public partial class frmTransactionSuccess : BaseFormKiosk
    {
        ...
        private int successScreenTimeout = 5;
        //int ticks = 0;
        public frmTransactionSuccess(string message, string point, string trxNo, string source, bool isWaiverPending)
        {
            ...
            InitializeComponent();
            this.BackgroundImage = ThemeManager.GetBackgroundImageTwo(ThemeManager.CurrentThemeImages.SuccessBackgroundImage);

            this.Size = this.BackgroundImage.Size;
            btnClose.BackgroundImage = ThemeManager.CurrentThemeImages.DoneButton;
            btnClose.Size = btnClose.BackgroundImage.Size;
            btnClose.Location = new System.Drawing.Point((this.Width - this.btnClose.Width) / 2, (this.Height - this.btnClose.Height - 10));
            ...
            {
                lblTrxNumber.Text = MessageContainerList.GetMessage(KioskStatic.Utilities.ExecutionContext, 4470, trxNo); //Transaction # : &1
                lblTrxNumber.Visible = true;
            }
            //timer1.Start();

            successScreenTimeout = ParafaitDefaultContainerList.GetParafaitDefault<int>(KioskStatic.Utilities.ExecutionContext, "SUCCESS_SCREEN_TIMEOUT", 5);
        ...
        public override void KioskTimer_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                int tickSecondsRemaining = 0;
                if (TimeOut.AbortTimeOut(this))
                {
                    KioskTimerInterval(successScreenTimeout * 1000);
                    tickSecondsRemaining = kioskTimer.Interval;
                }
                else
                {
