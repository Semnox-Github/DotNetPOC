    ...
    public partial class frmCardTransaction : BaseFormKiosk
    {
        ...
        private void frmNewcard_FormClosing(object sender, System.Windows.Forms.FormClosingEventArgs e)
        {
            ...
            if (billAcceptor != null && billAcceptor.StillProcessing())
            {
                ...
                log.Info("billAcceptor stillProcessing. reset timer clock");
                log.LogMethodExit();
                return;
            }
        ...
        private void SetReceiptPrintOptions()
        {
            ...
            string deliveryMode = ParafaitDefaultContainerList.GetParafaitDefault(executionContext, "KIOSK_RECEIPT_DELIVERY_MODE");
            kioskTransaction.RececiptDeliveryMode = KioskTransaction.KioskReceiptDeliveryMode.NONE;
            if (option.Equals("A"))
            {
                string message;
                message = MessageContainerList.GetMessage(utilities.ExecutionContext, 5012);
        ...
        private void InitializeBillAndNoteAcceptors()
        {
            ...
            try
            {
                ...
                if (commonBA != null && kioskTransaction.GetTotalPaymentsReceived() > 0)
                {
                    ...
                    //if (rowProduct["product_type"].ToString().Equals("VARIABLECARD"))//GGG test
            ...
            if (paymentModeDTO != null)
            {
                if (paymentModeDTO.IsCreditCard == true)
                {
                    ...
                    try
                    {
                        ...
                        //Credit card payment. Please wait...
                        DisplayMessageLine(MessageContainerList.GetMessage(executionContext, "Credit card payment.") + " " + MessageContainerList.GetMessage(executionContext, 684));
                        Application.DoEvents();
        ...
        private void BillAcceptorDataReceived()
        {
            ...
            try
            {
                ...
                if (noteRecd)
                {
                    ...
                    {
                        string errMsg = ex.Message + " " + MessageContainerList.GetMessage(executionContext, 441);
                        KioskStatic.logToFile("Error in billAcceptorDataReceived: " + errMsg);
                        log.Error(ex);
        ...
        private void CoinAcceptorDataReceived()
        {
            ...
            try
            {
                ...
                if (coinRecd)
                {
                    ...
                    {
                        string errMsg = ex.Message + " " + MessageContainerList.GetMessage(executionContext, 441);
                        KioskStatic.logToFile("Error in CoinAcceptorDataReceived: " + errMsg);
                        log.Error(ex);
                        frmOKMsg.ShowUserMessage(errMsg);
                    }
                    SetCancelButtonText();
        ...
        //            {
        //                coinMessage = MessageContainerList.GetMessage(executionContext, 424);
        ...
        //            }
        //        }
        //        double moneyReceived = kioskTransaction.GetTotalPaymentsReceived();
        //        if (moneyReceived <= 0)
        //        {
        ...
        //            Application.DoEvents();
        //            // check again if any money has been inserted at the last minute
        ...
        //                    lblTimeOut.Text = "";
        //                else
        //                    lblTimeOut.Text = MessageContainerList.GetMessage(executionContext, "Process Order");
        //            }
        //        }
            ...
            lock (thisLock)
            {
                try
                {
                    ...
                    if (kioskTransaction.GetTotalPaymentsReceived() >= kioskTransaction.GetTrxNetAmount())
                    {
                        ...
                        log.Info(msg);
                        //TimerMoney.Stop();
                        string infoMsg = MessageContainerList.GetMessage(utilities.ExecutionContext, 1008);
                        DisplayMessageLine(infoMsg);
        ...
        private void RestartValidators()
        {
            ...
            {
                KioskStatic.logToFile("RestartValidators - skip restart, cancel Button Pressed");
            }
            btnHome.Enabled = true;
            btnCancel.Enabled = true;
            //TimerMoney.Start();
        ...
        private void CancelAction(bool cancelButtonClicked)
        {
            ...
            try
            {
                    ...
                    && kioskTransaction.GetTotalPaymentsReceived() > 0)
                {
                    ...
                    if (kioskTransaction.SelectedProductType == KioskTransaction.GETNEWCARDTYPE)
                    {
                        ...
                        {
                            DialogResult dryn = f.ShowDialog();
                            if (dryn == System.Windows.Forms.DialogResult.No)
                            {
                                abort = AttemptToIssueNewVariableCardForPartialAmount();
                            }
                            else if (dryn == System.Windows.Forms.DialogResult.Yes)
                            {
                                abort = 1;
                            }
                            else
                    ...
                    else if (kioskTransaction.SelectedProductType == KioskTransaction.GETCHECKINCHECKOUTTYPE)
                    {
                        ...
                        {
                            DialogResult dryn = fop.ShowDialog();
                            if (dryn == System.Windows.Forms.DialogResult.Yes)
                            {
                                abort = 1;
                            }
                            else if (dryn == System.Windows.Forms.DialogResult.No)
                            {
                                abort = 0;
                            }
                            else
                    ...
                    if (abort == 1)
                    {
                        ...
                        string msg = "Abort transaction... Money entered: " + kioskTransaction.GetTotalPaymentsReceived().ToString(AMOUNTFORMATWITHCURRENCYSYMBOL);
                        KioskStatic.logToFile(msg);
                        log.Info(msg);
                        DisplayMessageLine(msg);
                        string transacionNumber = kioskTransaction.GetTransactionNumber;
                        kioskTransaction.ClearTransaction(frmOKMsg.ShowUserMessage);
                        frmOKMsg.ShowUserMessage(MessageContainerList.GetMessage(executionContext, 5080, transacionNumber)); //Please contact our staff. Keep the transaction Number: &1 for future reference 
            ...
            catch (Exception ex)
            {
                ...
                frmOKMsg.ShowOkMessage(ex.Message, true);
            }
            log.LogMethodExit();
        }
        ...
        private int toggleAcceptorMessage = 0;
        private string billMessage = "";
        private string coinMessage = "";
        private double totSecs = 0;
        public override void KioskTimer_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                totSecs += this.kioskTimer.Interval / 1000.0;
                ...
                {
                    //rececived money no timeout
                    Application.DoEvents();
                    totSecs = 0;
                    if (receivedMoney == false)
                    {
                        KioskStatic.logToFile("KioskTimer_Tick, rececived money no timeout");
                        receivedMoney = true;
                    }
                }
                if (cancelButtonPressed == false)
        ...
        private void CheckMoneyAcceptorStatus()
        {
            log.LogMethodEntry();
            if (billAcceptor != null && (KioskStatic.billAcceptorDatareceived == false || billAcceptor.Working == false))
            {
            ...
            //        && ((KioskStatic.spCoinAcceptor != null && KioskStatic.spCoinAcceptor.IsOpen) || microSPDevice))
            //    {
            //        coinMessage = MessageContainerList.GetMessage(executionContext, 424);
            //        KioskStatic.logToFile("Coin acceptor timeout / offline");
            //        log.Info("Coin acceptor timeout / offline");
            ...
            //    if (coinAcceptorTimeout % 5 == 0)
            //    {
            //        if ((KioskStatic.spCoinAcceptor != null && KioskStatic.spCoinAcceptor.IsOpen) || microSPDevice)
            //        {
            //            if (coinAcceptor != null)
            //                coinAcceptor.checkStatus();
            //        }
        ...
        private void UpdateMessages()
        {
            ...
            else
            {
                ...
                toggleAcceptorMessage++;
                if (toggleAcceptorMessage > 30)
                    toggleAcceptorMessage = -30;
            }
            log.LogMethodExit();
        }
        ...
        void ShowThankYou(bool receiptPrinted, bool receiptEmailed)
        {
            ...
            try
            {
                ...
                if (kioskTransaction.SelectedProductType == KioskTransaction.GETCHECKINCHECKOUTTYPE)
                {
                    ...
                    string productName = KioskHelper.GetProductName(checkInLine.ProductID);
                    message = MessageContainerList.GetMessage(utilities.ExecutionContext, 4149,
                                                                   (checkInLine != null ? productName : ""));
        ...
        private void closeForm(bool cancelButtonClicked)
        {
            log.LogMethodEntry(cancelButtonClicked);
            KioskStatic.logToFile("CloseForm in Card Transaction form");
            if (billAcceptor != null && billAcceptor.StillProcessing())
            {
                Application.DoEvents();
                totSecs = 0;
            ...
            {
                //int directCashFormIndex = GetDirectCashFormIndex();
                int lowerLimit = 0;//(directCashFormIndex == 1 ? 0 : 1);
        ...
        private void SetCustomizedFontColors()
        {
            ...
            try
            {
                ...
                this.BackgroundImage = ThemeManager.GetBackgroundImage(ThemeManager.CurrentThemeImages.CardTransactionBackgroundImage);
        ...
        private void DisplayValidtyMsg(int productId, string selectedEntitlementType)
        {
            ...
            else
            {
                ...
                this.pnlProductSummary.Size = new System.Drawing.Size(pnlProductSummary.Width, 385);
                this.flpCartProducts.Size = new System.Drawing.Size(flpCartProducts.Width, 300);
                this.bigVerticalScrollCartProducts.Size = new System.Drawing.Size(bigVerticalScrollCartProducts.Width, 300);
            }
            log.LogMethodExit();
        }
        ...
        private void SetCancelButtonText()
        {
            ...
            try
            {
                if (kioskTransaction != null && kioskTransaction.GetTotalPaymentsReceived() > 0)
                {
                    btnCancel.Text = MessageContainerList.GetMessage(executionContext, "Cancel");
                }
                else
                {
                    btnCancel.Text = MessageContainerList.GetMessage(executionContext, "Back");
                }
            }
            catch (Exception ex)
            {
