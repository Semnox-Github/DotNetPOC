 ...
 *2.150.7.0   29-Nov-2023      Suraj Pai K         Kiosk Transaction View Enhancement- Print, Refund, Print Pending, Issue Pending Card options added
 ********************************************************************************************/
using System;
...
namespace Parafait_Kiosk
{
    public partial class frmKioskTransactionView : BaseForm
    {
        ...
        public frmKioskTransactionView(bool managerCardFlag = false)
        {
            ...
            try
            {
                this.BackgroundImage = ThemeManager.GetBackgroundImage(ThemeManager.CurrentThemeImages.TransactionViewBackgroundImage);
        ...
        private void ShowOrHideRefundButton()
        {
            ...
            else
            {
                int xLoc = 30;
                int xLoc1 = 415;
                this.btnPrev.Location = new System.Drawing.Point(xLoc1, btnPrev.Location.Y);
                int xLoc2 = xLoc1 + btnPrev.Width + xLoc;
                this.btnPrintReceipt.Location = new System.Drawing.Point(xLoc2, btnPrintReceipt.Location.Y);
                int xLoc3 = xLoc2 + btnRefund.Width + xLoc;
                this.btnPrintPending.Location = new System.Drawing.Point(xLoc3, btnPrintPending.Location.Y);
                int xLoc4 = xLoc3 + btnPrintReceipt.Width + xLoc;
                this.btnIssueTempCard.Location = new System.Drawing.Point(xLoc4, btnIssueTempCard.Location.Y);
        ...
        private void btnPrintReceipt_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                KioskTransaction kioskTransaction = new KioskTransaction(KioskStatic.Utilities, trxIdList[0]);
                SetReceiptPrintOptions(kioskTransaction);

                if (kioskTransaction.RececiptDeliveryMode == KioskTransaction.KioskReceiptDeliveryMode.PRINT)
                {
                ...
                else if (kioskTransaction.RececiptDeliveryMode == KioskTransaction.KioskReceiptDeliveryMode.EMAIL)
                {
                    List<string> idList = kioskTransaction.GetTransactionCustomerIdentifierList();
                    if (idList != null && idList.Any())
                    {
                        TransactionEventContactsDTO transactionEventContactsDTO = new TransactionEventContactsDTO();
                        transactionEventContactsDTO.EmailId = idList[0];
                        transactionEventContactsDTO.MessageChannel = TransactionPaymentLink.MessageChannel.EMAIL;
                        transactionEventContactsDTO.TransactionId = kioskTransaction.GetTransactionId;
                        bool emailed = kioskTransaction.EmailTransactionReceipt(transactionEventContactsDTO, frmOKMsg.ShowUserMessage);
                        if (emailed == true)
                        {
                            string msg = MessageContainerList.GetMessage(utilities.ExecutionContext, 5013);  //Transaction receipt is emailed to you
                    ...
                    {
                        string msg = MessageContainerList.GetMessage(utilities.ExecutionContext, 5103); //Sorry unable to proceed. Email details are missing
                        ValidationException vExecp = new ValidationException(msg);
                        throw vExecp;
                    }
                }
                else
                {
                    string msg = MessageContainerList.GetMessage(utilities.ExecutionContext, 5104); // Receipt delivery mode is not set up
                    ValidationException vExecp = new ValidationException(msg);
                    throw vExecp;
                }
