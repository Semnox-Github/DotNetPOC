/********************************************************************************************
 * Project Name - CenterEdge  
 * Description  - Games class This would hold the games that are configured in the system. 
 *                        This would return the active games in the Parafait system. 
 *  
 **************
 **Version Log
 **************
 *Version     Date             Modified By               Remarks          
 *********************************************************************************************
 0.0         24-Sep-2020       Girish Kundar             Created : CenterEdge  REST API
 ********************************************************************************************/
using System.Collections.Generic;

namespace Semnox.Parafait.ThirdParty.CenterEdge
{
    public  class Game
    {
        private static readonly Semnox.Parafait.logging.Logger log = new Semnox.Parafait.logging.Logger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);
        private int skippedRecords;
        private int totalRecordCount;
        private List<GameDTO> gamesList;

        public Game()
        {
            log.LogMethodEntry();
            gamesList = new List<GameDTO>();
            skippedRecords = 0;
            totalRecordCount = 1;
            log.LogMethodExit();
        }

        public Game(int skipped, int totalCount)
        {
            log.LogMethodEntry(totalCount, skipped);
            this.skippedRecords = skipped;
            this.totalCount = totalCount;
            log.LogMethodExit();
        }
        public int skipped { get { return skippedRecords; } set { skippedRecords = value; } }
        public int totalCount { get { return totalRecordCount; } set { totalRecordCount = value; } }
        public List<GameDTO> games { get { return gamesList; } set { gamesList = value; } }
    }
}
