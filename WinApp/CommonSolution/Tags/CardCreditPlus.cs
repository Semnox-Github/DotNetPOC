...
namespace Semnox.Parafait.Tags
{
    ...
    public class CardCreditPlusList
    {
        ...
        /// <param name="sqlTrx"></param>
        /// <returns></returns>
        public double GetCPAttributeCPBalance(int customerId, string creditPlusType, DateTime fromDate, DateTime toDate, int siteId, SqlTransaction sqlTrx = null )
        {
            log.LogMethodEntry(customerId, creditPlusType, fromDate, toDate, siteId, sqlTrx);
            double returnValue = 0;
            System.TimeSpan diff = toDate - fromDate;
            int totalDays = (int)diff.Days;
            CardCreditPlusDataHandler cardCreditPlusDataHandler = new CardCreditPlusDataHandler();
            Dictionary<DateTime, double> balanceSheet = cardCreditPlusDataHandler.GetCPAttributeCPBalance(customerId, creditPlusType, fromDate, toDate, siteId, sqlTrx);
            double previousdaybalanceValue = 0;
            for (int i = 0; i < totalDays; i++)
            {
                if (balanceSheet.ContainsKey(fromDate.AddDays(i)))
                    previousdaybalanceValue = balanceSheet[fromDate.AddDays(i)];
                else
                    balanceSheet.Add(fromDate.AddDays(i), previousdaybalanceValue);
            }
            foreach (KeyValuePair<DateTime, double> balanceEntry in balanceSheet)
            {
                returnValue = returnValue + balanceEntry.Value;
            }
            log.LogMethodExit(Math.Round(returnValue/totalDays,2));
            return Math.Round(returnValue / totalDays, 2);
        }
