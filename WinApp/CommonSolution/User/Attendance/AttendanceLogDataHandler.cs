...
namespace Semnox.Parafait.User
{
    public class AttendanceLogDataHandler
    {
        ...
        private static readonly Dictionary<AttendanceLogDTO.SearchByParameters, string> DBSearchParameters = new Dictionary<AttendanceLogDTO.SearchByParameters, string>
        {
                ...
                {AttendanceLogDTO.SearchByParameters.ORIGINAL_LOG_ID, "al.OriginalAttendanceLogId"},
                {AttendanceLogDTO.SearchByParameters.REQUEST_STATUS, "al.RequestStatus"},
                {AttendanceLogDTO.SearchByParameters.STATUS, "al.Status"}
            };
        ...
        private List<SqlParameter> BuildSQLParameters(AttendanceLogDTO attendanceLogDTO, string loginId, int siteId)
        {
            ...
            parameters.Add(dataAccessHandler.GetSQLParameter("@UserId", attendanceLogDTO.UserId,true));
            parameters.Add(dataAccessHandler.GetSQLParameter("@Remarks", attendanceLogDTO.Remarks));
            parameters.Add(dataAccessHandler.GetSQLParameter("@Notes", attendanceLogDTO.Notes));
                    ...
                    if (DBSearchParameters.ContainsKey(searchParameter.Key))
                    {
                        joiner = (count == 0) ? string.Empty : " and ";
                        {
                            if (searchParameter.Key.Equals(AttendanceLogDTO.SearchByParameters.ATTENDANCE_LOG_ID) ||
                                ...
                                searchParameter.Key.Equals(AttendanceLogDTO.SearchByParameters.ORIGINAL_LOG_ID) ||
                            ...
                            else if (searchParameter.Key.Equals(AttendanceLogDTO.SearchByParameters.TO_DATE))
                            {
                                ...
                                parameters.Add(new SqlParameter(dataAccessHandler.GetParameterName(searchParameter.Key), DateTime.ParseExact(searchParameter.Value, "yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture)));
                                     ...
                                     searchParameter.Key.Equals(AttendanceLogDTO.SearchByParameters.STATUS) ||
                                     searchParameter.Key.Equals(AttendanceLogDTO.SearchByParameters.REQUEST_STATUS))
                            {
                                query.Append(joiner + DBSearchParameters[searchParameter.Key] + " =  " + dataAccessHandler.GetParameterName(searchParameter.Key));
