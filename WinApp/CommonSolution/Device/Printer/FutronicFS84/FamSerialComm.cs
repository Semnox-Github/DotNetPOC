...
namespace Futronic
{
    public class FamSerialComm
    {
        ...
        private static void MineRegistryForPortName(string strStartKey, Hashtable oTargetMap, string[] oPortNamesToMatch)
        {
            ...
            {
                log.Error("Error Occurred at MineRegistryForPortName() method", e);
        ...
        public byte PrepareComPort()
        {
            ...
            if (!bConnected && m_nBaudrate != m_nMaxBaudrate)
            {
                ...
                catch (Exception e)
                {
                    ...
                    m_SerialPort.Close();
                    m_SerialPort = null;
                    log.Error("Error while executing PrepareComPort() method", e);
                    log.LogMethodExit(null, "Throwing Exception -" + e.Message);
                    log.LogMethodExit(1);
                    return 1;
                }
            ...
            if (!bConnected)
            {
                ...
                log.Error("Error while executing PrepareComPort() method");
                log.LogMethodExit(null, "Throwing Exception -" + m_strComErrorMessage);
                log.LogMethodExit(1);
                return 1;
            }
        ...
        public bool ReadRxBufferEndBytes()
        {
            ...
            {
                m_strComErrorMessage = e.Message;
                log.Error("Error while executing ReadRxBufferEndBytes() method", e);
                log.LogMethodExit(null, "Throwing Exception -" + m_strComErrorMessage);
                log.LogMethodExit(false);
                return false;
            }
        ...
        public byte CommunicateWithFAC(byte nCommand, uint param1, uint param2, byte nFlag, byte[] RxCmd, byte[] TxBuf, byte[] RxBuf)
        {
            ...
            {
                m_strComErrorMessage = e.Message;
                log.Error("Error while executing CommunicateWithFAC() method", e);
                log.LogMethodExit(null, "Throwing Exception -" + m_strComErrorMessage);
                log.LogMethodExit(1);
                return 1;
            }
            ...
            while (true)
            {
                ...
                {
                    m_strComErrorMessage = "Timeout to read data";
                    log.LogMethodExit(null, "Error :  -" + m_strComErrorMessage);
                    log.LogMethodExit(1);
                    return 1;
                }
                System.Threading.Thread.Sleep(10);
        ...
        public bool ReadFileByBlock(byte[] lpBuffer, int nNumberOfBytesToRead, bool bShowPer)
        {
            ...
            while (nTotal > 0)
            {
                ...
                {
                    m_strComErrorMessage = e.Message;
                    log.Error("Error while executing ReadFileByBlock() method", e);
                    log.LogMethodExit(null, "Throwing Exception -" + m_strComErrorMessage);
                    log.LogMethodExit(false);
                    return false;
                }
                ...
                if (nBytesRead != nBytesToRead)
                {
                    ...
                    log.Error("Error :nBytesRead != nBytesToRead");
                    log.LogMethodExit(null, "Throwing Exception -" + m_strComErrorMessage);
                    log.LogMethodExit(false);
                    return false;
                }
        ...
        public bool WriteFileByBlock(byte[] lpBuffer, int nNumberOfBytesToWrite, bool bShowPer)
        {
            ...
            while (nTotal > 0)
            {
                ...
                {
                    m_strComErrorMessage = e.Message;
                    log.Error("Error at WriteFileByBlock() method ",e);
                    log.LogMethodExit(null, "Throwing Exception -" + m_strComErrorMessage);
                    log.LogMethodExit(false);
                    return false;
                }
