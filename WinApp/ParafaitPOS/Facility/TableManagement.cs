...
namespace Semnox.Parafait.Facility
{
    public class TableManagement
    {
        ...
        TableLayoutPanel tblPanelTables;
        FlowLayoutPanel flpFacilities;
        Panel panelTables;
        ContextMenuStrip ctxOrderContextTableMenu;
        Form form;
        ...
        public TableManagement()
        {
            ...
            log.Debug("Starts-TableManagement()");//Added for logger function on 08-Mar-2016
            form = new Form();
            ...
            log.Debug("Ends-TableManagement()");//Added for logger function on 08-Mar-2016
        }

        private void SwitchOnClick(object sender, EventArgs e)
        {
            log.Debug("Starts-SwitchOnClick()");//Added for logger function on 08-Mar-2016
            ...
            try
            {
                ParafaitUtils.ExternalInterfaces.SwitchOn(dt.Rows[0]);
            }
            catch (Exception ex)
            {
            ...
            log.Debug("Ends-SwitchOnClick()");//Added for logger function on 08-Mar-2016
        }

        private void SwitchOffClick(object sender, EventArgs e)
        {
            log.Debug("Starts-SwitchOffClick()");//Added for logger function on 08-Mar-2016
            DataTable dt = Utilities.executeDataTable(@"select * from FacilityTables where tableId = @tableId", new SqlParameter("@tableId", (sender as ToolStripMenuItem).Tag));
            try
            {
                ParafaitUtils.ExternalInterfaces.SwitchOff(dt.Rows[0]);
            }
            catch (Exception ex)
            {
                POSUtils.ParafaitMessageBox(ex.Message);
            ...
            log.Debug("Ends-SwitchOffClick()");//Added for logger function on 08-Mar-2016
        }

        void initializeTableLayout()
        {
            log.Debug("Starts-initializeTableLayout()");//Added for logger function on 08-Mar-2016
            ...
            log.Debug("Ends-initializeTableLayout()");//Added for logger function on 08-Mar-2016
        }

        void rb_CheckedChanged(object sender, EventArgs e)
        {
            log.Debug("Starts-rb_CheckedChanged()");//Added for logger function on 08-Mar-2016
            loadTables();
            log.Debug("Ends-rb_CheckedChanged()");//Added for logger function on 08-Mar-2016
        }
